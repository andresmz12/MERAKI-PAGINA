import Link from 'next/link'
import Image from 'next/image'
import { formatPrice } from '@/lib/utils'

interface Propiedad {
  id: string
  slug: string
  titulo: string
  precio: number
  moneda: string
  tipo: string
  operacion: string
  estado: string
  ciudad: string
  barrio?: string | null
  area?: number | null
  habitaciones?: number | null
  banos?: number | null
  imagenes: string[]
  destacada: boolean
}

export default function PropertyCard({ propiedad }: { propiedad: Propiedad }) {
  const mainImage = propiedad.imagenes[0]

  const operacionLabel = propiedad.operacion === 'venta' ? 'VENTA' : 'ARRIENDO'

  const estadoBadge =
    propiedad.estado === 'vendido'
      ? { label: 'Vendido', classes: 'bg-red-600 text-white' }
      : propiedad.estado === 'arrendado'
      ? { label: 'Arrendado', classes: 'bg-gray-600 text-white' }
      : null

  return (
    <Link href={`/propiedades/${propiedad.slug}`} className="property-card group block">
      <div className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 hover:-translate-y-1">

        {/* Image */}
        <div className="card-image relative aspect-[4/3] overflow-hidden">
          {mainImage ? (
            <Image
              src={mainImage}
              alt={propiedad.titulo}
              fill
              className="object-cover"
              sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"
            />
          ) : (
            <div className="w-full h-full bg-gradient-to-br from-[#0A0A0A] to-[#1A1A1A] flex items-center justify-center">
              <span className="font-display text-[#C9A84C] text-4xl font-bold tracking-widest opacity-40">M</span>
            </div>
          )}

          {/* Hover overlay */}
          <div className="absolute inset-0 bg-[#111111]/50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
            <span className="font-sans bg-[#C9A84C] text-[#0A0A0A] px-6 py-2.5 text-xs font-bold tracking-widest uppercase">
              Ver Propiedad
            </span>
          </div>

          {/* Badges */}
          <div className="absolute top-3 left-3">
            <span className="font-sans bg-[#C9A84C] text-[#0A0A0A] text-xs font-bold px-3 py-1.5 tracking-wider">
              {operacionLabel}
            </span>
          </div>
          {estadoBadge && (
            <div className="absolute top-3 right-3">
              <span className={`font-sans text-xs font-bold px-3 py-1.5 tracking-wider ${estadoBadge.classes}`}>
                {estadoBadge.label}
              </span>
            </div>
          )}
          {propiedad.destacada && !estadoBadge && (
            <div className="absolute top-3 right-3">
              <span className="font-sans bg-[#0A0A0A] text-[#C9A84C] text-xs font-bold px-3 py-1.5 tracking-wider">
                DESTACADA
              </span>
            </div>
          )}
        </div>

        {/* Content */}
        <div className="p-5">
          <div className="font-display text-[#C9A84C] text-2xl font-semibold mb-1">
            {formatPrice(propiedad.precio, propiedad.moneda)}
            {propiedad.operacion === 'arriendo' && (
              <span className="font-sans text-sm font-normal text-[#8B8B9E]">/mes</span>
            )}
          </div>

          <h3 className="font-sans font-semibold text-[#2C2C3E] text-base leading-snug mb-2 line-clamp-2">
            {propiedad.titulo}
          </h3>

          <p className="font-sans text-[#8B8B9E] text-xs mb-4">
            <svg className="w-3.5 h-3.5 inline mr-1 -mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/>
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/>
            </svg>
            {propiedad.ciudad}{propiedad.barrio ? `, ${propiedad.barrio}` : ''}
          </p>

          {/* Stats row */}
          {(propiedad.habitaciones || propiedad.banos || propiedad.area) && (
            <div className="flex items-center gap-4 py-3 border-t border-[#C9A84C]/20 text-xs text-[#8B8B9E] font-sans">
              {propiedad.habitaciones && (
                <div className="flex items-center gap-1.5">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/>
                  </svg>
                  <span>{propiedad.habitaciones} hab.</span>
                </div>
              )}
              {propiedad.banos && (
                <div className="flex items-center gap-1.5">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 14v3m4-3v3m4-3v3M3 21h18M3 10h18M3 7l9-4 9 4M4 10h16v11H4V10z"/>
                  </svg>
                  <span>{propiedad.banos} baños</span>
                </div>
              )}
              {propiedad.area && (
                <div className="flex items-center gap-1.5 ml-auto">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 8V4m0 0h4M4 4l5 5m11-1V4m0 0h-4m4 0l-5 5M4 16v4m0 0h4m-4 0l5-5m11 5l-5-5m5 5v-4m0 4h-4"/>
                  </svg>
                  <span>{propiedad.area} m²</span>
                </div>
              )}
            </div>
          )}
        </div>
      </div>
    </Link>
  )
}
